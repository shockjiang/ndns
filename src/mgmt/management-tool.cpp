                         ...
                         const time::seconds& ttl)
{
  ...
  if (ndnsType == NDNS_RAW) {
    Block tmp = ndn::dataBlock(ndns::tlv::Rr, content.c_str(), content.length());
                         ...
                         Name dskCertName)
{
  ...
  if (regex1->match(data->getName())) {
    ...
    BOOST_ASSERT(regex2->match(keyLocator) == true);
    if (keyName == regex2->expand("\\1\\2")) {
      shared_ptr<Data> tmp = data;
      ...
      if (zoneName != regex1->expand("\\1")) {
        Name tmp1 = regex1->expand("\\1").getSubName(zoneName.size());
        Name tmp2 = regex1->expand("\\2");
        name = zoneName;
        name.append("KEY");
        name.append(tmp1);
        name.append(tmp2);
        name.append("ID-CERT");
        name.append(tmp->getName().get(-1));
      }

      data = make_shared<Data>();
      data->setName(name);
      data->setContent(tmp->getContent());
...
void
ManagementTool::getKeyNameList(const Name& identity, std::vector<Name>& keyNameList)
{
  m_keyChain.getAllKeyNamesOfIdentity(identity, keyNameList, false);
  m_keyChain.getAllKeyNamesOfIdentity(identity, keyNameList, true);
}

void
ManagementTool::getCertNameList(const Name& identity, std::vector<Name>& certNameList)
{
  std::vector<Name> keyList;
  getKeyNameList(identity, keyList);
