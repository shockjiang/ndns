...
public:
   ...
   *  Other complicated situations can be handled by the another addRrSet() function.
   *
  ...
  addRrSet(const Name& zoneName,
           ...
           Name dskCertName = DEFAULT_CERT,
           const time::seconds& ttl = DEFAULT_RR_TTL);

  /** @brief add rrset to the NDNS local database
   *
   *  This one is capable of adding any data to the rrset as long as the supplied data is valid.
  ...
  /** @brief it generates an output like DNS zone file. Reference:
  ...
  /** @brief it lists all existing zones within this name server.
   *
   *  @param os the ostream to print information to
    */
  void
...
private:
  ...
  /** @brief get the whole key name list of given identity
   */
  void
  getKeyNameList(const Name& identity, std::vector<Name>& keyNameList);

  /** @brief get the whole certificate name list of given identity
   */
  void
  getCertNameList(const Name& identity, std::vector<Name>& certNameList);
