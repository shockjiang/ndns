...
public:
  ...
  getPublicKeyOfData(const Data& data) {
    ...
    //std::cout << klName.toUri() << std::endl;
    ...
    rrset.setLabel(label);
    rrset.setType(label::CERT_RR_TYPE);
    //std::cout << zoneName.toUri() << " " << label.toUri() << std::endl;
...
BOOST_FIXTURE_TEST_CASE(AddRrSet5, ManagementToolFixture)
{
  ...
  BOOST_CHECK_NO_THROW(
    m_tool.addRrSet(parentZoneName, label1, label::NS_RR_TYPE, NDNS_AUTH, -1, "",
                    dskCert->getName()));
...
//  ; zWm+6IoZA6mV1J2O5U0AkbllRVZ+vZPI/lWGzCtXK6jfAgMBAAE=
  m_tool.listZone(zoneName, std::cout, true);
...
//  puPJKCOSQHspQHOhnEy9Ee9U
  m_tool.getRrSet(zoneName, label, label::NS_RR_TYPE, std::cout);
